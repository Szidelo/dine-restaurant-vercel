...
const MenuHighlights = ({ data }) => {
	const { title, desc, menu_list } = data;
	return (
		<section className="menu d-flex justify-content-center">
			<div className="container-fluid container-xl row mx-auto">
				<div className="col-12 col-xl-6 d-flex justify-content-start align-items-start flex-column gap-4 pb-5">
					<img className="mx-auto mx-xl-0 mb-5" src={headingSvg} alt="" />
					<p className="heading-l text-white w-80 text-center text-xl-start">{title}</p>
					<p className="body-1 text-white w-80 text-center text-xl-start">{desc}</p>
				</div>
					...
					{menu_list.map((item) => (
						<div key={item.title} className="menu-item d-flex">
							...
							<div className="ms-md-4 text-center text-sm-start">
								<h4 className="heading-m text-white">{item.title}</h4>
								<h4 className="body-2 text-white">{item.desc}</h4>
							</div>
						</div>
					))}
				</div>
